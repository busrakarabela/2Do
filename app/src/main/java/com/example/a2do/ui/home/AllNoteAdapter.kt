...
import com.example.a2do.databinding.NoteRowBinding
import com.example.a2do.model.Note

class AllNoteAdapter: ListAdapter<Note, AllNoteAdapter.ViewHolder>(DIFF_CALLBACK) {
    ...
    class ViewHolder(val binding: NoteRowBinding, onNoteListClickListener: OnNoteListClickListener,changeStatus: ChangeStatus): RecyclerView.ViewHolder(binding.root) {
        init {
            binding.root.setOnClickListener {
                onNoteListClickListener.onClick(binding.note!!)
        ...
        companion object {
            fun create(inflater: LayoutInflater, parent: ViewGroup, onNoteListClickListener: OnNoteListClickListener,changeStatus: ChangeStatus): ViewHolder {
                val itemRowBinding = NoteRowBinding.inflate(inflater, parent, false)
                return ViewHolder(itemRowBinding,onNoteListClickListener,changeStatus)
            }
        }
